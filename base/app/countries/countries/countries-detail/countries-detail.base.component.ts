...
filteredItemscountryName:any = [];
isAutoSuggestCallFiredcountryName: boolean = false;
...
showMessage(config:any){
 ...
 this.bindLookupFields();
	...
	autoSuggestSearchcountryName(event?: any, col?: any,url?:any) {
 }
	normalize(data:any){
    return data;
  }
 ...
 onChangelog() {
	...
	attachInfiniteScrollForAutoCompletecountryName(fieldName:string) {
    ...
    const autoSuggestScrollSubscription = windowYOffsetObservable.subscribe((scrollPos: number) => {
      if ((tracker.offsetHeight + scrollPos >= tracker.scrollHeight)) {
        this.isAutoSuggestCallFiredcountryName = false;
          if(this.filteredItemscountryName.length  >= this.autoSuggestPageNo * BaseAppConstants.defaultPageSize){
            this.autoSuggestPageNo = this.autoSuggestPageNo + 1;
          }
         const methodName: any = `autoSuggestSearchcountryName`
        let action: Exclude<keyof CountriesDetailBaseComponent, ' '> = methodName;
        this[action]();
      }
    });
