  ...
  isValidObject(obj: any, config: any) {
    ...
    config.columns.forEach((col: any) => {
      if (col.name !== 'jid') {
        ...
        if (obj[col.name] && col.nonAcceptedChars) {
          let nonaccChars = col.nonAcceptedChars.split(',');
          if (commonAllowedValCheck(nonaccChars, false, { value: obj[col.name] })) {
            errors = { ...errors, ...{ [col.name]: `${col.label} contains not allowed Values` } };
          }
        }
        if (obj[col.name] && col.acceptedChars) {
          let accChars = col.acceptedChars.split(',');
          if (commonAllowedValCheck(accChars, true, { value: obj[col.name] })) {
            errors = { ...errors, ...{ [col.name]: `${col.label} is not matching with allowed values` } };
          }
        }
      }
    });
